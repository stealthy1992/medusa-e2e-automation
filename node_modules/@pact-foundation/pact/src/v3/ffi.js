...
const setRequestDetails = (interaction, req) => {
...
const setResponseDetails = (interaction, res) => {
    interaction.withStatus(MatchersV3.reify(res.status));
    if (MatchersV3.isStatusCodeMatcher(res.status)) {
        interaction.withResponseMatchingRules(JSON.stringify((0, matchingRules_1.convertStatusMatcherToFFI)(res.status)));
    }
    (0, ramda_1.forEachObjIndexed)((v, k) => {
        if (Array.isArray(v)) {
            v.forEach((header, index) => {
...
const contentTypeFromHeaders = (headers, defaultContentType) => {
