...
function computeReplaceUrlSettings(snapshot, replaceResourceUrl) {
    if (!replaceResourceUrl || !utils.types.has(snapshot, 'url'))
        return undefined;
...
function buildUrlList(snapshotUrl, finalResources, originalSnapshotResource) {
    // CDT (snapshotUrl) is always placed first so index 0 is predictably the CDT.
    // If the snapshot URL also resolves to a real resource (e.g. navigating to a raw image),
    // append it again at the end so it becomes a separate entry that the CDT can depend on.
...
function buildHashedEntries({ urls, urlToIndex, directUrls, dependencyMap, finalResources, originalSnapshotResource, }) {
    return urls.map((url, i) => {
        ...
        let deps;
        if (i === 0) {
            deps = directUrls.map(u => urlToIndex.get(u)).filter((i) => i !== undefined);
        }
        else if ((_a = dependencyMap[url]) === null || _a === void 0 ? void 0 : _a.length) {
            deps = dependencyMap[url].map(u => urlToIndex.get(u)).filter((i) => i !== undefined);
        }
        else {
            deps = [];
        }
        // The last entry is the original resource (e.g. image) when snapshotUrl was also a resource
        const hash = originalSnapshotResource && i === urls.length - 1 && url === urls[0]
            ? originalSnapshotResource
            : finalResources[url];
        return (0, resource_1.makeHashedEntry)({ url, hash, rootDomSnapshot: i === 0 ? true : undefined }, deps);
    });
}
...
function buildDependencyMap({ snapshot, snapshotResources, processedDomResource, frameResources, replaceResourceUrlsSettings, }) {
    const frameDependencyMapping = frameResources.reduce((deps, resources, index) => {
        ...
        if (utils.types.has(frameSnapshot, 'cdt')) {
...
function makeCreateRenderTarget({ processResources, logger: mainLogger, replaceResourceUrl }) {
    return async function createRenderTarget({ snapshot, settings, logger = mainLogger, }) {
        ...
        // Must be computed BEFORE processSnapshotResources, which mutates snapshot.url to the replaced value.
        // Using the already-mutated URL would produce {from: replaceResourceUrl, to: replaceResourceUrl} (a no-op).
        ...
        const finalResources = replaceResourceUrl
    ...
    async function processSnapshotResources({ snapshot, settings, logger, }) {
        ...
        const [snapshotResources, ...frameResources] = await Promise.all([
            processResources({
                resources: Object.fromEntries([
                    ...((_a = snapshot.resourceUrls) !== null && _a !== void 0 ? _a : [])
                        .filter(url => ((settings === null || settings === void 0 ? void 0 : settings.skipRootHtmlResource) ? url !== snapshot.url : true))
                        .map(url => {
        ...
        // Capture the original resource at snapshotUrl (e.g. an image) before the CDT blob overwrites it
        const originalSnapshotResource = utils.types.has(snapshot, 'url')
            ? snapshotResourcesMapping[snapshot.url]
            : undefined;
