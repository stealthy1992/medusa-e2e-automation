...
import {HttpProxyResolver} from './execute-request-through-http-proxy'
...
export interface FullSocks5ProxyOptions {
  ...
  /**
   * CLIENT-SIDE: per-request HTTP CONNECT proxy resolver.
   * Separate from `proxyServers` (SERVER-SIDE SOCKS5-chain load-balancing).
   * Consulted first; a `null`/`undefined` return falls through to the SOCKS5-chain
   * branch (which is typically unconfigured on the client side, ending in DIRECT).
   */
  httpProxyResolver: HttpProxyResolver
