...
async function startEgTunnelService({
  ...
  useTcpTunnel = true,
  // PAC inputs (all optional; if all unset and matching env vars are unset,
  // no PAC is loaded — behaviour identical to today, zero regression).
  // Resolution priority within each: option wins over env var.
  // Across the three: pacFile > pacScript > pacUrl (handled inside
  // loadPacResolver). Errors at startup throw a typed PacError —
  // see src/utils/pac-errors.js for the contract.
  pacFile = process.env.APPLITOOLS_EG_TUNNEL_PAC_FILE,
  pacScript = process.env.APPLITOOLS_EG_TUNNEL_PAC_SCRIPT,
  pacUrl = process.env.APPLITOOLS_EG_TUNNEL_PAC_URL,
} = {}) {
  if (useTcpTunnel) {
  ...
  // Load PAC source (if configured) BEFORE starting the SOCKS5 server.
  // - Returns the compiled `findProxyForURL` function on success.
  // - Returns null when no PAC input is set (fast path — no QuickJS boot).
  // - Throws a PacError on any startup failure (file read, URL fetch, compile).
  //   The error propagates up to the caller and (in `require.main === module`
  //   mode) causes `process.exit(1)` — fail-fast by design.
  const findProxyForURL = await loadPacResolver({
    pacFile,
    pacScript,
    pacUrl,
    logger: serviceLogger,
