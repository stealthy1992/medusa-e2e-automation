...
import { Request, Headers, Response, getFetchForVersion, validateOptionsForVersion } from './fetch.js';
...
export function makeReq(baseOptions) {
    // Validate HTTP/2 incompatibilities eagerly at construction time when httpVersion is set in base options.
    // Validation is scoped to the undici implementation — see validateOptionsForVersion.
...
export async function req(input, ...requestOptions) {
    ...
    async function buildRequest(input, opts, requestController, getBodySize) {
        var _a, _b, _c, _d, _e;
    ...
    async function singleReq(input, options) {
        var _a, _b, _c;
        // Validate HTTP/2 incompatibilities on the merged options — catches per-request httpVersion: 2 usage
        // (e.g. makeReq({})(url, {httpVersion: 2, proxy: ...})) that bypass the makeReq construction-time check.
        validateOptionsForVersion(options.httpVersion, options);
        const { fetch: versionedFetch, getBodySize } = getFetchForVersion(options.httpVersion, options.useDnsCache, options.proxy);
        const fetch = (_a = options.fetch) !== null && _a !== void 0 ? _a : versionedFetch;
        ...
        while (true) {
            ...
            const request = await buildRequest(input, options, requestController, getBodySize);
            try {
                ...
                const envHttpVersion = utils.general.getEnvValue('HTTP_VERSION')
                    ? parseInt(utils.general.getEnvValue('HTTP_VERSION'))
                    : undefined;
                const enrichedOptions = {
                    ...options,
                    httpVersion: (_c = options.httpVersion) !== null && _c !== void 0 ? _c : envHttpVersion,
                };
                // Success - return response
                response = await afterResponse({ request, response, options: enrichedOptions });
                return response;
            }
            catch (error) {
