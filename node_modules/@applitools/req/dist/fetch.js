...
var cachify = general.cachify;
var getFetchImpl = cachify(async (httpVersion) => {
  let fetchImpl;
  let getBodySize2 = () => void 0;
  if (httpVersion === "2") {
    const module = await import("./fetch-http2.js");
    fetchImpl = module.fetchImpl;
    getBodySize2 = module.getBodySize;
  } else {
    const module = await Promise.resolve().then(() => (init_src(), src_exports));
    fetchImpl = module.default;
    getBodySize2 = () => void 0;
  }
  return { fetchImpl, getBodySize: getBodySize2 };
});
var defaultFetch = async (...args) => {
  const httpVersion = general.getEnvValue("HTTP_VERSION") || "";
  const { fetchImpl } = await getFetchImpl(httpVersion);
  return fetchImpl(...args);
};
var defaultGetBodySize = async (...args) => {
  const httpVersion = general.getEnvValue("HTTP_VERSION") || "";
  const { getBodySize: getBodySize2 } = await getFetchImpl(httpVersion);
  return getBodySize2(...args);
};
var fetch_default = defaultFetch;
var getBodySize = defaultGetBodySize;
function getFetchForVersion(httpVersion, useDnsCache, proxy) {
  if (httpVersion === 2 || httpVersion === 1) {
    const httpVersionKey = httpVersion === 2 ? "2" : "";
    const extraOpts = httpVersion === 2 ? { useDnsCache, proxy } : {};
    return {
      fetch: async (url, opts = {}) => {
        const { fetchImpl } = await getFetchImpl(httpVersionKey);
        return fetchImpl(url, { ...opts, ...extraOpts });
      },
      getBodySize: async (...args) => {
        const { getBodySize: getBodySize2 } = await getFetchImpl(httpVersionKey);
        return getBodySize2(...args);
      }
    };
  }
  return { fetch: defaultFetch, getBodySize: defaultGetBodySize };
}
function validateOptionsForVersion(httpVersion, options) {
  var _a, _b, _c;
  if (httpVersion === 2) {
    if (options.keepAliveOptions)
      throw new Error("HTTP/2 is not compatible with keepAliveOptions");
    const nodeMajor = parseInt((_c = (_b = (_a = process.versions) === null || _a === void 0 ? void 0 : _a.node) === null || _b === void 0 ? void 0 : _b.split(".")[0]) !== null && _c !== void 0 ? _c : "0");
    if (nodeMajor > 0 && nodeMajor < 20)
      throw new Error("HTTP/2 requires Node.js v20 or later");
  }
}
export {
  Headers,
