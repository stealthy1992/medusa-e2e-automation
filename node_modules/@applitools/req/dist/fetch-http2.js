...
var cachify = utils.general.cachify;
var disableHttpAgentReuse = process.env.APPLITOOLS_DISABLE_AGENT_CACHIFY;
var cachifiedCreateAgent = cachify((options) => {
  return new Agent({
    allowH2: options.allowH2,
    maxHeaderSize: 16384,
    keepAliveTimeout: options.keepAliveTimeout,
    keepAliveMaxTimeout: options.keepAliveMaxTimeout,
    connect: { timeout: 6e4, lookup: options.useDnsCache ? lookupWithCache : void 0 },
    bodyTimeout: 3e5,
    headersTimeout: 3e5
  });
});
function createProxyAgent(options) {
  const isHttpsProxy = new URL(options.proxyUrl).protocol === "https:";
  return new ProxyAgent({
    uri: options.proxyUrl,
    token: options.proxyAuthHeader,
    allowH2: options.allowH2,
    maxHeaderSize: 16384,
    keepAliveTimeout: options.keepAliveTimeout,
    keepAliveMaxTimeout: options.keepAliveMaxTimeout,
    ...
    bodyTimeout: 3e5,
    headersTimeout: 3e5,
    // TODO: make configurable — preserved for parity with HTTP/1.1 path (agent.ts)
    // HACK: undici's ProxyAgent (lib/dispatcher/proxy-agent.js:131) does
    // `buildConnector({ ...opts.requestTls })` — it spreads only requestTls and
    // never propagates opts.allowH2 to the inner CONNECT connector. The inner
    // connector then defaults allowH2 to false (lib/core/connect.js:87) and hard-
    // codes ALPNProtocols to ['http/1.1'], silently downgrading h2-through-proxy
    // to h1.1. buildConnector destructures `allowH2` directly from its first
    // argument, so embedding it inside requestTls is enough to opt in. Remove
    // when undici fixes the propagation, or when this codebase upgrades to v8+
    // (which flipped the default to true and masks the bug).
    requestTls: { rejectUnauthorized: options.rejectUnauthorizedOrigin, allowH2: options.allowH2 },
    ...isHttpsProxy && {
      // TODO: make configurable — preserved for parity with HTTP/1.1 path (agent.ts)
      proxyTls: { rejectUnauthorized: options.proxyTlsRejectUnauthorized }
    }
  });
}
var cachifiedCreateProxyAgent = cachify(createProxyAgent);
function makeDispatcher(opts) {
  return function dispatcher(url) {
    const isHttps = url.protocol === "https:";
    const allowH2 = isHttps;
    const resolvedProxy = utils.types.isFunction(opts.proxy) ? opts.proxy(url) : opts.proxy;
    if (resolvedProxy) {
      ...
      const authToken = proxyUrl.username ? `Basic ${Buffer3.from(`${decodeURIComponent(proxyUrl.username)}:${decodeURIComponent(proxyUrl.password)}`).toString("base64")}` : void 0;
      proxyUrl.username = "";
      proxyUrl.password = "";
      const proxyAgentOptions = {
        proxyUrl: proxyUrl.href,
        proxyAuthHeader: authToken,
        allowH2,
        keepAliveTimeout: 3e4,
        keepAliveMaxTimeout: 3e5,
        useDnsCache: opts.useDnsCache,
        rejectUnauthorizedOrigin: false,
        proxyTlsRejectUnauthorized: false
      };
      if (disableHttpAgentReuse) {
        return createProxyAgent(proxyAgentOptions);
      }
      return cachifiedCreateProxyAgent(proxyAgentOptions);
    }
    const agentOptions = {
      allowH2,
      keepAliveTimeout: 3e4,
...
function getBodyLength(url, body, headers) {
  ...
  if (typeof body === "string")
    return String(Buffer3.byteLength(body));
  if (body && body.size)
    return String(body.size);
  return void 0;
}
var fetchImpl = async (url, options = {}) => {
  var _a, _b, _c, _d, _e;
  let finalUrl;
  ...
  if (!finalOptions.dispatcher) {
    ...
    const useDnsCache = (_e = finalOptions.useDnsCache) !== null && _e !== void 0 ? _e : utils.general.getEnvValue("USE_DNS_CACHE") === "true";
    finalOptions.dispatcher = makeDispatcher({ useDnsCache, proxy: finalOptions.proxy })(urlObj);
  }
  const isStream = finalOptions.body && (typeof finalOptions.body.pipe === "function" || typeof finalOptions.body[Symbol.asyncIterator] === "function");
  if (isStream) {
    finalOptions.duplex = "half";
  }
  const detectedSize = getBodyLength(url, finalOptions.body, finalOptions.headers);
  if (detectedSize) {
    const contentLengthHeader = Object.keys(finalOptions.headers).find((h2) => h2.toLowerCase() === "content-length");
  ...
  delete finalOptions.useDnsCache;
  delete finalOptions.proxy;
  try {
    const response = await undiciFetch(finalUrl, finalOptions);
    const nodeBody = response.body ? Readable.from(response.body) : void 0;
    return new Response(nodeBody, {
...
function getBodySize(input, opts) {
  var _a, _b;
  ...
  if (isBodyAllowed) {
    if (opts.size) {
      ...
      if (Buffer3.isBuffer(finalBody)) {
        ...
        finalSize = Buffer3.byteLength(finalBody);
