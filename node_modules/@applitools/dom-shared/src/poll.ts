...
function createPollResponse(state: PollState, {chunkByteLength = 0}: PollOptions = {}) {
  if (!state) {
    ...
    if (chunkByteLength) {
      if (!state.chunks) {
        try {
          const _nativeStringify =
            typeof window !== 'undefined' ? (window as any).__EYES__APPLITOOLS__?._stringify : undefined
          const stringified = (_nativeStringify ?? JSON.stringify)(state.value)
